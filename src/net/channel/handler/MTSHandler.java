    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (slea.available() > 0) {
            ...
            if (op == 2) { //put item up for sale
                ...
                long alt_prie = price * quantity;
                if(alt_prie > Integer.MAX_VALUE){
                    return;
                }
                ...
                Connection con = DatabaseConnection.getConnection();
                try {
                    PreparedStatement ps = con.prepareStatement("SELECT * FROM mts_items where seller = ? && itemid = ?");
                    ps.setInt(1, c.getPlayer().getId());
                    ps.setInt(2, id);
                    ResultSet rs = ps.executeQuery();
                    if(!rs.next()){
                        rs.close();
                        return; // HAX : DOES NOT ACTUALLY OWN THE ITEM.
                    }
