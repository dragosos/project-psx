...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private int team;
    private int pvpownerid = -1;
    private boolean isFarmerMonster = false;
    public List<client.MaplePet> locked_on_pets = new ArrayList<client.MaplePet>();
    public boolean monsterPet = false;
    public boolean moneyDropped = false;
    ...
    public short calculatePvpDamage(int rbs, int damage) {
        if (damage > 1218) { // random
            damage = (int) (1000 + Math.random() * 218);
        }
        damage += (rbs += Math.random() * (rbs * 1.5));
        return (short) damage;
    }
    ...
    public void damage(MapleCharacter from, int damage, boolean updateAttackTime) {
        ...
        if (from.getParty() != null) {
            ...
            attacker = new SingleAttackerEntry(from, from.getClient().getChannelServer(), this);
        }
    ...
    public MapleCharacter killBy(MapleCharacter killer) {
		...
		if (toSpawn != null && killer != null && killer.getMap() != null) {
    ...
    public boolean applyStatus(MapleCharacter from, final MonsterStatusEffect status){
        return applyStatus(from, status, false, 20000);
    }
    ...
    public boolean applyStatus(MapleCharacter from, final MonsterStatusEffect status, boolean poison, long duration, boolean venom) {
        if(from.getBuffedValue(MapleBuffStat.ELEMENTAL_RESET) != null){
            return true; // effectiveness GONE
        }
    ...
    public void applyCustomCurse(final MonsterStatusEffect eff, long duration){
        final Runnable cancelTask = new Runnable() {

            @Override
            public void run() {
                if (isAlive()) {
                    MaplePacket packet = MaplePacketCreator.cancelMonsterStatus(getObjectId(), eff.getStati());
                    map.broadcastMessage(packet, getPosition());
                    if (getController() != null && !getController().isMapObjectVisible(MapleMonster.this)) {
                        getController().getClient().getSession().write(packet);
                    }
                }
                for (MonsterStatus stat : eff.getStati().keySet()) {
                    stati.remove(stat);
                }
                setVenomMulti(0);
                eff.cancelDamageSchedule();
            }
        };
        for(MonsterStatus stat : eff.getStati().keySet()){
            stati.put(stat, eff);
        }
        MaplePacket packet = MaplePacketCreator.applyMonsterStatus(getObjectId(), eff);
        map.broadcastMessage(packet, getPosition());
        eff.setCancelTask(TimerManager.getInstance().schedule(cancelTask, duration));
    }
    ...
    private class SingleAttackerEntry implements AttackerEntry {
        ...
        public SingleAttackerEntry(MapleCharacter from, ChannelServer cserv, MapleMonster mob) {
    ...
    public boolean hasPvpOwner() {
        return map.getCharacterById(pvpownerid) != null;
    }

    public void setPvpOwner(int cid) {
        pvpownerid = cid;
    }
    ...
    public void setFarmer(boolean y){
        isFarmerMonster = y;
    }
    
    public boolean isFarmerMonster(){
        return isFarmerMonster;
    }
