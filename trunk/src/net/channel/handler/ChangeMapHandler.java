...
public final class ChangeMapHandler extends AbstractMaplePacketHandler {
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if(chr.getTrade() != null){
            chr.dropMessage(1, "Nice try.");
            chr.getClient().disconnect();
            return;
        }
        chr.ismapChanging = true;
        try{
        if (slea.available() == 0) { //is this even used?
            ...
            if(c.getPlayer().getPQ() != null){ // player dead!
                c.getPlayer().getPQ().removeMember(c.getPlayer());
            }
            ...
            if (targetid != -1 && !chr.isAlive()) {
                ...
                if (executeStandardPath) {
                    ...
                    if(chr.getDonator().isActive()){
                        chr.setHp(chr.getMaxHp() / 2);
                        chr.changeMap(chr.getMap());
                        c.announce(MaplePacketCreator.enableActions());
                        chr.setRates();
                        return;
                    } else if (wheel && chr.getItemQuantity(5510000, false) > 0) {
                 ...
                 MapleMap to = c.getChannelServer().getMapFactory(c.getPlayer().getGMode()).getMap(targetid);
                 chr.changeMap(to, to.getPortal(0));
            } else if (targetid != -1 && !chr.isGM()) {
