...
public final class DistributeAPHandler extends AbstractMaplePacketHandler {
    ...
    static int addHP(MapleClient c) {
        ...
        if(MaxHP >= (99999 - 20)){
            player.completeAchievement(MapleAchievementLibrary.MAXING_HP);
        }
        return MaxHP;
    }
    ...
    static int addMP(MapleClient c) {
        ...
        MapleJob job = player.getJob();
        if (player.getHpMpApUsed() > 9999 || player.getMaxMp() >= 99999) {
            return MaxMP;
        }
        if (job.isA(MapleJob.WARRIOR) || job.isA(MapleJob.DAWNWARRIOR1) || job.isA(MapleJob.ARAN1)) {
            MaxMP += 2;
        ...
        if(MaxMP >= (99999 - 18)){
            player.completeAchievement(MapleAchievementLibrary.MAXING_MP);
        }
        return MaxMP;
    }
