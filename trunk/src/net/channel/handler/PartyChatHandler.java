...
public final class PartyChatHandler extends AbstractMaplePacketHandler {
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
       // System.out.println("CHAT :" + slea);
        MapleCharacter player = c.getPlayer();
        slea.readInt();//v88 probably timestamp
        int type = slea.readByte(); // 0 for buddys, 1 for partys, 2 for guilds
        ...
        try {
            if (type == 0) {
                ...
                for(ChannelServer cs : ChannelServer.getAllInstances()){
                    for(MapleCharacter players : cs.getPlayerStorage().getAllCharacters()){
                    if(players.getGuildId() == c.getPlayer().getGuildId() && players.getId() != c.getPlayer().getId()){
                    if(players != null && players.getId() != c.getPlayer().getId()){
                        players.announce(MaplePacketCreator.multiChat(c.getPlayer().getName(), chattext, 2));
                    }
                    }
                    }
                }
