...
public final class PlayerInteractionHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        //System.out.println("PLAYER INTERACTION : " + slea);
        ...
        if (mode == Action.CREATE.getCode()) {
            ...
            if (createType == 3) {// trade
                ...
                if (chr.getMapId() > 910000000 && chr.getMapId() < 910000023 || itemId > 5030000 && itemId < 5030012 || itemId > 5140000 && itemId < 5140006) {
                    if (createType == 4) {
                        if(1==1){
                            chr.dropMessage(1, "This doesn't work. Make a Hired Merchant instead.");
            ...
            if (chr.getItemQuantity(chr.getInventory(type).getItem(slot).getItemId(), false) < bundles) {
                return;
            }
            short perBundle = slea.readShort();
            int price = slea.readInt();
            if (perBundle < 0 || perBundle * bundles > 2000 || bundles < 0 || price < 0 || (long)(perBundle * price) > Integer.MAX_VALUE) {
                return;
            }
            IItem ivItem = chr.getInventory(type).getItem(slot);
            IItem sellItem = ivItem.copy();
            if(sellItem.getType() != 1){
                c.getPlayer().dropMessage(1, "Unfortunately, Fredrick is broken. Please only use Equips for now.");
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
            ...
            MaplePlayerShop shop = chr.getPlayerShop();
            if(shop != null){
                System.out.println("Blocked player shop.");
                return; // no shops allowedd
            }
            HiredMerchant merchant = chr.getHiredMerchant();
