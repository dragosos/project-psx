...
public final class TakeDamageHandler extends AbstractMaplePacketHandler {
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (damagefrom != -3) {
            ...
            oid = slea.readInt();
            attacker = (MapleMonster) player.getMap().getMapObject(MapleMapObjectType.MONSTER, oid);
        ...
        if(player.getDonator().isActive() && damage > 0){
            for(client.MaplePet p : player.getPets()){
                double percent = 0.20;
                damage *= 0.20;
                if((int)10.0 * Math.random() > 7){
                player.getMap().broadcastMessage(MaplePacketCreator.petChat(player.getId(), p.getPosition(), 1, "Master! Let me block part of the blow.."));
                }
            }
        }
        ...
        if (damage > 0 && !player.isHidden()) {
            if (attacker != null && !attacker.isBoss()) {
                ...
                } else if(player.getParty() == null && player.getDarkFlare() != null){
                   int bounceDamage = damage;
                   int compareHP = (int)(attacker.getMaxHp() * 0.25);
                   if(bounceDamage > compareHP){
                       bounceDamage = compareHP;
                   }
                   
                   attacker.damage(player, bounceDamage, false);
                   attacker.getMap().broadcastMessage(MaplePacketCreator.damageMonster(attacker.getObjectId(), bounceDamage));
                   attacker.getMap().broadcastMessage(MaplePacketCreator.showMonsterHP(attacker.getObjectId(), (int)(attacker.getHp() / attacker.getMaxHp())));
                } else if(player.getParty() != null){
                    boolean hasDarkFlare = false;
                    for(net.world.MaplePartyCharacter mpc : player.getParty().getMembers()){
                        MapleCharacter ptyPlayer = mpc.getPlayer();
                        if(ptyPlayer.getDarkFlare() != null){
                            hasDarkFlare = true;
                        }
                    }
                    if(hasDarkFlare){
                        int bounceDamage = damage;
                        int compareHP = (int)(attacker.getMaxHp() * .25);
                        if(bounceDamage > compareHP){
                            bounceDamage = compareHP;
                        }
            ...
            if (player.getBuffedValue(MapleBuffStat.MAGIC_GUARD) != null && mpattack == 0 || player.getBuffedValue(MapleBuffStat.MAGIC_SHIELD) != null) {
            //    int mploss = (int) (damage * (player.getBuffedValue(MapleBuffStat.MAGIC_GUARD).doubleValue() / 100.0));
                double value = player.getBuffedValue(MapleBuffStat.MAGIC_GUARD) != null ? 0.8 : 0.3;
                int mploss = (int)((double) damage * 0.8); // adding 80% to mploss
