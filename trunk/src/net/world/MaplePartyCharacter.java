...
public class MaplePartyCharacter implements Serializable {
    ...
    public MapleCharacter getPlayer(){
        return ChannelServer.getCharacterFromAllServers(id);
    }
    ...
    public boolean isPartyBonusEligible(){
        boolean eligible = false;
        if(getPlayer().getMap().getMapName().toLowerCase().contains("lion king")){
            for(MaplePartyCharacter mpc : this.getPlayer().getParty().getMembers()){
                if(mpc.getMapId() == this.getMapId()){
                    eligible = true;
                    break;
                }
            }
