...
public class AbstractPlayerInteraction {
    ...
    protected MapleMap getWarpMap(int map) {
        ...
        if (getPlayer().getEventInstance() == null) {
            target = c.getChannelServer().getMapFactory(c.getPlayer().getGMode()).getMap(map);
        }
        return target;
    }
    ...
    public int getPlayerCount(int mapid) {
        return c.getChannelServer().getMapFactory(c.getPlayer().getGMode()).getMap(mapid).getCharacters().size();
    }
