...
public class MapleInventoryManipulator {
    public static boolean addRing(MapleCharacter chr, int itemId, int ringId) {
        ...
        chr.getClient().announce(MaplePacketCreator.addInventorySlot(type, nEquip, false));
        return true;
    }
    ...
    public static boolean addById(MapleClient c, int itemId, short quantity, String owner, int petid, byte flag, long expiration) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!ItemConstants.isRechargable(itemId)) {
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && (eItem.getOwner().equals(owner) || owner == null)) {
                                ...
                                if (eItem.getItemId() >= 5000000 && eItem.getItemId() <= 5000100) {
                                    eItem.setExpiration(-1); // 3 months :)
                                }
                ...
                while (quantity > 0 || ItemConstants.isRechargable(itemId)) {
                    ...
                    if (newQ != 0) {
                        ...
                        c.announce(MaplePacketCreator.addInventorySlot(type, nItem, false));
            ...
            c.announce(MaplePacketCreator.addInventorySlot(type, nEquip, false));
        } else {
    ...
    public static boolean addFromDrop(MapleClient c, IItem item, boolean show) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            c.announce(MaplePacketCreator.addInventorySlot(type, item, hasPotential));
        } else {
            return false;
        }
        if (show) {
    ...
    public static void equip(MapleClient c, byte src, byte dst){
        equip(c.getPlayer(), src, dst, false);
    }
    
    public static void equip(MapleCharacter c, byte src, byte dst, boolean clone) {
        clone = false;
        Equip source = (Equip) c.getInventory(MapleInventoryType.EQUIP).getItem(src);
        Equip target = (Equip) c.getInventory(MapleInventoryType.EQUIPPED).getItem(dst);
        if(source.getPetId() > -1){
            return;
        }
        if (source == null || !MapleItemInformationProvider.getInstance().canWearEquipment(c, source)) {
            c.announce(MaplePacketCreator.enableActions());
            return;
        } else if ((((source.getItemId() >= 1902000 && source.getItemId() <= 1902002) || source.getItemId() == 1912000) && c.isCygnus()) || ((source.getItemId() >= 1902005 && source.getItemId() <= 1902007) || source.getItemId() == 1912005) && !c.isCygnus()) {// Adventurer taming equipment
            return;
        }
        ...
        if (source.getRingId() > -1) {
            c.getRingById(source.getRingId()).equip();
        }
        if (dst == -6) { // unequip the overall
            IItem top = c.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -5);
            if (top != null && isOverall(top.getItemId())) {
                if (c.getInventory(MapleInventoryType.EQUIP).isFull()) {
                    c.announce(MaplePacketCreator.getInventoryFull());
                    c.announce(MaplePacketCreator.getShowInventoryFull());
                    return;
                }
                unequip(c.getClient(), (byte) -5, c.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot());
            }
        } else if (dst == -5) {
            final IItem bottom = c.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -6);
            if (bottom != null && isOverall(source.getItemId())) {
                ...
                unequip(c.getClient(), (byte) -6, c.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot());
            }
        } else if (dst == -10) {// check if weapon is two-handed
            IItem weapon = c.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -11);
            if (weapon != null && MapleItemInformationProvider.getInstance().isTwoHanded(weapon.getItemId())) {
                ...
                unequip(c.getClient(), (byte) -11, c.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot());
            }
        } else if (dst == -11) {
            IItem shield = c.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -10);
            if (shield != null && MapleItemInformationProvider.getInstance().isTwoHanded(source.getItemId())) {
                ...
                unequip(c.getClient(), (byte) -10, c.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot());
            }
        }
        if (dst == -18) {
            if (c.getMount() != null) {
                c.getMount().setItemId(source.getItemId());
            }
        }
        if (source.getItemId() == 1122017) {
            c.equipPendantOfSpirit();
        } else if(source.getItemId() == 1002419 || source.getItemId() == 1032033){ // Mark of the BETA
          c.announce(MaplePacketCreator.showOwnBuffEffect(2022108, 11));
          c.getMap().broadcastMessage(c, MaplePacketCreator.showBuffeffect(c.getId(), 2022108, 11), false); // HT nine spirit
          switch(source.getItemId()){
              case 1002419:
                  c.announce(MaplePacketCreator.serverNotice(5, "By equipping the Mark Of The Beta, your pets will recognize you as their leader. They will aid you in battle with physical attacks."));  
                  break;
              case 1032033:
                  c.announce(MaplePacketCreator.serverNotice(5, "By equipping the Protector Rock, pets will recognize you as their leader. They will help protect you in battle."));
                  break;
          }
        }
        //1112413, 1112414, 1112405 (Lilin's Ring)
        source = (Equip) c.getInventory(MapleInventoryType.EQUIP).getItem(src);
        target = (Equip) c.getInventory(MapleInventoryType.EQUIPPED).getItem(dst);
        c.getInventory(MapleInventoryType.EQUIP).removeSlot(src);
        if (target != null) {
            c.getInventory(MapleInventoryType.EQUIPPED).removeSlot(dst);
        }
        source.setPosition(dst);
        c.getInventory(MapleInventoryType.EQUIPPED).addFromDB(source);
        if (target != null) {
            target.setPosition(src);
            c.getInventory(MapleInventoryType.EQUIP).addFromDB(target);
        }
        if (c.getBuffedValue(MapleBuffStat.BOOSTER) != null && isWeapon(source.getItemId())) {
            c.cancelBuffStats(MapleBuffStat.BOOSTER);
        }
        c.announce(MaplePacketCreator.moveInventoryItem(MapleInventoryType.EQUIP, src, dst, (byte) 2));
        c.forceUpdateItem(MapleInventoryType.EQUIPPED, source);
        c.equipChanged();
    }
    ...
    public static void drop(MapleClient c, MapleInventoryType type, byte src, short quantity) {
        if(c.getPlayer().getTrade() != null){
            c.getPlayer().ban("Trying to dupe..", true);
            return;
        }
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        if (src < 0) {
        ...
        IItem source = c.getPlayer().getInventory(type).getItem(src);
        if(c.getPlayer().isMWLB()){
            c.getPlayer().dropMessage(1, "As a Maple-Leaf Brigadier, you cannot drop items for the regular public. It will now disappear.");
            MapleInventoryManipulator.removeFromSlot(c, type, src, quantity, false);
            c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), source, c.getPlayer().getPosition());
            c.announce(MaplePacketCreator.enableActions());
            return;
        }
