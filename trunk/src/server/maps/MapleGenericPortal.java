...
public class MapleGenericPortal implements MaplePortal {
    ...
    public void enterPortal(MapleClient c) {
        ...
        if (getScriptName() != null) {
            ...
            MapleMap to = c.getChannelServer().getMapFactory(c.getPlayer().getGMode()).getMap(getTargetMapId());
            MaplePortal pto = to.getPortal(getTarget());
            if (pto == null) {// fallback for missing portals - no real life case anymore - intresting for not implemented areas
                c.getPlayer().dropMessage(1, "This portal is not coded. Please report to the administrator that portal '" + getScriptName() + "' is not working!");
                pto = to.getPortal(0);
            }
