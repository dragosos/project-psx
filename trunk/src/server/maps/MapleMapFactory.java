...
public class MapleMapFactory {
    ...
    private int channel;
    private GameMode gMode = GameMode.getById(0); // by default, normal.
    public ArrayList<Integer> bossMaps = new ArrayList<Integer>();
    ...
    public MapleMap getMap(int mapid) {
        ...
        if (map == null) {
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        if (type.equals("n")) {
                            ...
                            map.addMonsterSpawn(monster.getId(), monster.getPosition(), mobTime, 0, (short) monster.getFh(), (short) monster.getRx0(), (short) monster.getRx1(), (short) monster.getCy(), (short) monster.getF());
                ...
                for (MapleData life : mapData.getChildByPath("life")) {
                    ...
                    if (myLife instanceof MapleMonster) {
                        ...
                        if (mobTime == -1) //does not respawn, force spawn once
                        {
                            ...
                            if(ChannelServer.getInstance(channel).getCalendarManager().getCurrentEvent().equals(CalendarEventsLibrary.CHRISTMAS)){
                            if((int)(10.0 * Math.random()) > 7 && monster.getStats().getLevel() <= 40 && monster.getStats().getLevel() > 10){
                                map.addMonsterSpawn(9400712, new java.awt.Point(monster.getPosition().x += (int)150.0 * Math.random(), monster.getPosition().y), mobTime, team, (short) monster.getFh(), (short) monster.getRx0(), (short) monster.getRx1(), (short) monster.getCy(), (short) monster.getF()); // snowman
                            }
                            }
                            map.addMonsterSpawn(monster.getId(), monster.getPosition(), mobTime, team, (short) monster.getFh(), (short) monster.getRx0(), (short) monster.getRx1(), (short) monster.getCy(), (short) monster.getF());
                        }
                    } else {
                        map.addMapObject(myLife);
                    }
                ...
                map.setHPDec(MapleDataTool.getIntConvert("decHP", mapData, 0, 1));
                map.setHPDecProtect(MapleDataTool.getIntConvert("protectItem", mapData, 0));
                // TODO: Add Survival Reward that protects you from HP DEC..:o!
