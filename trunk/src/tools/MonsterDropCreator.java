...
public class MonsterDropCreator {
    ...
    protected static List<Integer> existingItemsCache = new ArrayList<Integer>();
	...
	MapleData data = MapleDataProviderFactory.getDataProvider(new File(System.getProperty("wzpath") + "/monsterbook.wz")).getData("MonsterBook.img");
	...
	// MonsterBook
	System.out.println("Loading : Drops from MonsterBook.img.");
	...
	for (Pair<Integer, String> Pair : itemNameCache) {
		...
		for (Pair<Integer, MobInfo> Pair_ : mobCache) {
			...
			SQL.append(itemid).append(", "); // Itemid
...
//		    case 2070011: // Maple Throwing star
    ...
    public static boolean itemExists(int itemid){
        if(!existingItemsCache.isEmpty()){
            return existingItemsCache.contains(itemid);
        }
        boolean exists = false;
        MapleDataProvider Data = MapleDataProviderFactory.getDataProvider(new File(System.getProperty("wzpath") + "/String.wz"));
        MapleData cashData = Data.getData("Cash.img");
        MapleData consumeData = Data.getData("Consume.img");
        MapleData equipData = Data.getData("Eqp.img");
        MapleData etcData = Data.getData("Etc.img");
        MapleData insData = Data.getData("Ins.img");
        MapleData petData = Data.getData("Pet.img");
        for(MapleData cash : cashData.getChildren()){
            if(cash.getName().equals(String.valueOf(itemid))){
                exists = true;
            }
            if(!existingItemsCache.contains(Integer.parseInt(cash.getName())));
            existingItemsCache.add(Integer.parseInt(cash.getName()));
        }
        if(exists){
            return exists;
        }
        for(MapleData cons : consumeData.getChildren()){
            if(cons.getName().equals(String.valueOf(itemid))){
                exists = true;
                return exists;
            }
            if(!existingItemsCache.contains(Integer.parseInt(cons.getName()))){
            existingItemsCache.add(Integer.parseInt(cons.getName()));
        }
        }
        if(exists){
            return exists;
        }
        for(MapleData equip : equipData.getChildren()){
            if(equip.getName().equals(String.valueOf(itemid))){
                exists = true;
                return exists;
            }
            if(!existingItemsCache.contains(Integer.parseInt(equip.getName()))){
                existingItemsCache.add(Integer.parseInt(equip.getName()));
            }
        }
        if(exists){
            return exists;
        }
        for(MapleData etc : etcData.getChildren()){
            if(etc.getName().equals(String.valueOf(itemid))){
                exists = true;
                return exists;
            }
            if(!existingItemsCache.contains(Integer.parseInt(etc.getName()))){
                existingItemsCache.add(Integer.parseInt(etc.getName()));
            }
        }
        if(exists){
            return exists;
        }
        for(MapleData ins : insData.getChildren()){
            if(ins.getName().equals(String.valueOf(itemid))){
                exists = true;
                return exists;
            }
            if(!existingItemsCache.contains(Integer.parseInt(ins.getName()))){
                    existingItemsCache.add(Integer.parseInt(ins.getName()));
        }
        }
        if(exists){
            return exists;
        }
        for(MapleData pet : petData.getChildren()){
            if(pet.getName().equals(String.valueOf(itemid))){
                exists = true;
                return exists;
            }
            if(!existingItemsCache.contains(Integer.parseInt(pet.getName()))){
                existingItemsCache.add(Integer.parseInt(pet.getName()));
            }
        }
        if(exists){
            return exists;
